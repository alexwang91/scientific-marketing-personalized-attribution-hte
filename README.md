...
- `aurora-airpurifier-category-config.json` — category portfolio diagnostic (ref 17), fictional brand / SKUs: `report_type=category_portfolio`, plus `investment_plan` + `hte_validation` + `mmm` — demonstrates the SKU × marketing-module budget-frontier allocation engine (`investment_schema.py` / `hte_core.py` / `investment_engine.py` / `investment_charts.py` / `mmm_bridge.py`): where to spend, why, Qini/AUUC + decile validation, which cells are frozen for lack of evidence, and how confident each dollar is.
