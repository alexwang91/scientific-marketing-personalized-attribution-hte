...
报告分三档深度（`--depth`）：`quick` 只渲染决策关键章节，`standard`（默认）完整报告，
`deep` 在完整报告后追加一张**验证路线图**——把未决评审挑战、缺失台账、测试预测合并为
一张按「能否改变决策」排序的清单（只用配置里已有的数据，绝不新造数字）。
...
| `generate_report.py` | 决策备忘 HTML 生成器（v2）。**强制 provenance 合约**：任何非 sourced/assumed/derived/missing 的数字一律构建失败；含 5 张承载因果逻辑的交互图（ECharts）；三档深度 `--depth quick/standard/deep`（deep 追加验证路线图 §18） | **HTML 输出入口** |
