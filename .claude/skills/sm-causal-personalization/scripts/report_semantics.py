...
OPERATOR_STRINGS: dict[str, dict[str, str]] = {
    "en": {
        ...
        "inv_th_measurement": "Measurement",
        "inv_th_kpi": "KPI",
        # HTE validation core (hte_core.py) — why trust the tau numbers
        "inv_hte_heading": "HTE Validation Core (why trust the tau numbers)",
        "inv_hte_sub": "Validated uplift, model quality, and the tau distribution behind the budget math.",
        "inv_hte_gate_title": "HTE validation gate",
        "inv_hte_gate_note": "Method: {method} · holdout n: {n}. A cell is called validated only when its validation_ref clears every configured gate: AUUC (rank quality), calibration MAE (magnitude honesty), and interval coverage (uncertainty honesty — models are systematically overconfident, so intervals must prove their own coverage on holdout).",
        "inv_th_validation_ref": "Validation ref",
        "inv_th_learner": "Learner",
        "inv_th_auuc": "Qini / AUUC",
        "inv_th_calibration_mae": "Calibration MAE",
        "inv_th_coverage": "Interval coverage",
        "inv_th_passes": "Passes gate",
        "inv_th_decile": "Decile",
        "inv_th_predicted_tau": "Predicted tau",
        "inv_th_observed_lift": "Observed lift",
        "inv_th_gap": "Gap",
        "inv_th_tau_bucket": "Tau bucket",
        "inv_th_share": "Audience share",
        # MMM macro-calibration detail (charts from an already-fit source)
        "inv_mmm_sub": "Channel-level macro-model summary (PyMC-Marketing style) used to calibrate the SKU-level plan.",
        "inv_th_channel": "Channel",
        "inv_th_contribution": "Contribution",
        "inv_th_roas": "Posterior ROAS",
        "inv_th_interval": "90% interval",
        "inv_mmm_adstock": "Adstock / response",
        "inv_mmm_saturation": "Saturation",
        "inv_mmm_lift_calibration": "Lift calibration",
        # activation-card defaults (used when a cell doesn't set the field)
        "inv_default_owner": "Unassigned",
        "inv_default_flight": "Next planning cycle",
        "inv_default_kpi": "Incremental units",
        "inv_default_stop_rule": "Pause if measured marginal ROI falls below the required floor.",
        "inv_default_why": "Funded because its marginal ROI cleared the budget cutoff.",
        # report limits + CSV data-request template (replaces the never-funded wall)
        "inv_limits_heading": "What limits this report — and the data that unlocks the next run",
        "inv_limits_intro": "Of the {funded} funded cell(s), {assumption} are assumption-grade; {blocked} cell(s) could not be funded at all; holdout n = {holdout}. These numbers steer planning — they are not proof.",
        "inv_limits_blocked_line": "Not funded:",
        "inv_csv_heading": "CSV template — fill it in and the next run computes itself",
        "inv_csv_hint": "Pre-filled with today's cells; blank fields are exactly what's missing. tau_source must be one of randomized_hte / modeled_hte / mmm_calibrated_prior / expert_assumption; a randomized_hte row must also carry a validation_ref.",
        "inv_csv_download": "Download CSV",
        "inv_csv_copy": "Copy",
        # diagnosis investment-stance badges (right side of each finding card)
        "stance_invest": "Invest more",
        "stance_test": "Test small",
        "stance_hold": "Don't fund yet",
        "stance_fix": "Fix first, then fund",
        # per-business-line frontier panels
        "inv_frontier_by_group": "The same curve, split by business line",
        "inv_frontier_group_sub": "Each panel: cumulative gross profit as that line's spend grows — compare shapes to see which line still has room.",
        # tier drill-down
        "dash_tiers_drill_hint": "Click a tier to open its SKUs; click a SKU for its plays and this cycle's funded spend.",
        "tier_spend_word": "funded this cycle",
        "tier_sku_count_word": "SKU(s)",
        # narrative bridges under each chapter band (category dashboard)
        "cat_bridge_ch1": "The call first. Every chapter below answers one follow-up: why, against whom, how it lands, and why believe it.",
        "cat_bridge_ch2": "The last chapter said who gets money and how much — this one opens the books: what's broken in the category, and why the spend stops exactly where it does.",
        "cat_bridge_ch3": "The math is settled; now the play: which tier the money enters, which SKU, which module.",
        "cat_bridge_ch4": "The play is set; this chapter is execution — one owner and one stop-loss line per card, plus the data to collect for the next run.",
        "cat_bridge_ch5": "Finally, evidence and risk: which numbers stand on proof and which are still assumptions — folded by default, open what you need.",
        # 4P row labels inside the tier drill-down
        # D-dimension criterion 5: proxy must not condition on post-treatment
        # behaviour or a collider (the classic source of fake retargeting lift)
        "post_treatment_warn": "⚠ post-treatment proxy risk",
        "post_treatment_capped_note": "⚠-marked dimensions have a proxy that conditions on post-treatment behaviour (or a collider) — their heatmap grade is capped at Test: apparent lift there may be selection, not causation.",
        "dash_4p_product": "Product",
        "dash_4p_price": "Price",
        "dash_4p_place": "Place",
        "dash_4p_promotion": "Promotion",
        # portfolio verdict words (shared by document pills and dashboard cards)
        "verdict_grow": "Grow / Invest",
        "verdict_hold": "Hold",
        "verdict_harvest": "Harvest",
        "verdict_exit": "Exit / Plan succession",
        # interactive dashboard chrome (section titles/subtitles, detail panel)
        "dash_hero_portfolio": "Portfolio Verdict",
        "dash_top_action": "Top action",
        "dash_top_blocker": "Top blocker",
        "dash_detail_title": "Detail",
        "dash_detail_empty_title": "Select an item",
        "dash_detail_empty_hint": "Click a KPI, channel, heatmap cell, treatment, diagnosis, or evidence row to inspect its reasoning.",
        "dash_kpi_skus": "SKUs",
        "dash_kpi_grow": "Grow",
        "dash_kpi_exit": "Exit",
        "dash_kpi_missing": "Missing data",
        "dash_tiers_title": "Tier Map",
        "dash_tiers_sub": "Each tier: trend, who buys there, what moves them, and who we're up against.",
        "dash_diag_title": "Diagnosis Lenses",
        "dash_diag_sub": "Finding → evidence → cost if ignored → the move that fixes it; severity capped by evidence grade.",
        "dash_skus_title": "SKU Matrix",
        "dash_skus_sub": "Grouped by verdict — Grow first, Exit last. Click a card for the full reasoning.",
        "dash_handoff_title": "Deep-Dive Handoff",
        "dash_handoff_sub": "Grow-verdict SKUs — confirm, then run each through the single-SKU pipeline.",
        "dash_evidence_title": "Evidence Ledger",
        "dash_evidence_sub": "Sourced facts, assumptions, derivations, and missing values — nothing invented.",
        "dash_economics_title": "Economics",
        "dash_economics_sub": "Unit economics, CAC ceiling, and sensitivity levers.",
        "dash_channels_title": "Channel Screen",
        "dash_channels_sub": "Which channels can be defended before spend is unlocked.",
        "dash_dimensions_title": "D Dimension Table",
        "dash_dimensions_sub": "Buyer characteristics that predict incremental response, within the channels above.",
        "dash_heatmap_title": "Play matrix",
        "dash_suppression_title": "Who NOT to touch",
        "dash_suppression_sub": "Targeting these people burns money — exclusions are part of the play.",
        "dash_treatments_title": "Treatment Gates",
        "dash_treatments_sub": "Every action carries mechanism, guardrail, test, and gate.",
        "dash_budgets_title": "Budget release",
        "dash_budgets_sub": "Money unlocks stage by stage; each line names its release condition.",
        "dash_gates_sub": "Everything that must land before budget unlocks.",
        "dash_challenges_title": "Self-critique",
        "dash_challenges_sub": "Independent challenges to this analysis — open items block dependent spend.",
        "dash_map_title": "Causal Map",
        "dash_map_sub": "The dashboard keeps the reasoning sequence visible.",
        "dash_measurement_title": "Measurement Plan",
        # audience cards (ref 18: define -> size -> reach)
        "aud_heading": "Who exactly, how big, how to reach them (audience cards)",
        "aud_intro": "The channel screen says where to buy media. These cards say who to point it at: each audience is defined before a platform segment is named, sized as a chain (not one number), graded A-D, and given a reach path with its honest match quality.",
        "aud_role_word": "Causal role",
        "aud_role_persuadable": "Persuadable — the ones worth paying for",
        "aud_role_sure_thing": "Sure-thing — would buy anyway, suppress",
        "aud_role_lost_cause": "Lost cause — nothing moves them",
        "aud_role_sleeping_dog": "Sleeping dog — touching them backfires",
        "aud_role_unknown": "Unknown — needs a holdout to tell",
        "aud_who_word": "Who they are",
        "aud_size_word": "How big the pool is",
        "aud_reachable_word": "Reachable pool",
        "aud_persuadable_word": "Of which persuadable",
        "aud_persuadable_unknown": "Persuadable share unknown — run a holdout before scaling",
        "aud_grade_word": "Grade",
        "aud_worst_grade_note": "The whole card inherits its weakest link's grade.",
        "aud_factor_country_reachable": "Reachable in-country",
        "aud_factor_category_or_intent_share": "× in category / shopping now",
        "aud_factor_activation_match_rate": "× the proxy can actually reach",
        "aud_factor_eligibility_rate": "× eligible (shipping, policy)",
        "aud_factor_persuadable_share": "× persuadable (causal)",
        "aud_reach_word": "How to reach them",
        "aud_th_platform": "Platform",
        "aud_th_proxy": "Targeting proxy",
        "aud_th_match": "Match quality",
        "aud_th_leaks": "What leaks in",
        "aud_mq_high": "high",
        "aud_mq_medium": "medium",
        "aud_mq_low": "low",
        "aud_mq_unavailable": "can't target",
        "aud_suppression_word": "Exclude",
        "aud_measurement_word": "How we'd measure it",
        "aud_weakest_word": "Weakest assumption",
        "aud_grade_legend": "Grade: A verified · B category/search data · C third-party estimate · D assumption only (never a precise % on a D).",
        "aud_caption": "How to read: the pool narrows left to right; the number on the right is what's reachable, and its badge is the weakest grade in the chain. Persuadable comes only from a holdout — never from a platform segment.",
        "dash_audience_title": "Audience cards",
        "dash_audience_sub": "Who exactly, how big the pool is, and how to reach them — sized as a chain and graded.",
    },
    "zh": {
        "ch1_title": "1 · 拍板",
        ...
        "inv_th_measurement": "怎么测",
        "inv_th_kpi": "考核指标",
        "inv_hte_heading": "HTE 验证核心（tau 数字凭什么信）",
        "inv_hte_sub": "预算数字背后：验证过的增量、模型质量、tau 分布。",
        "inv_hte_gate_title": "HTE 验证门槛",
        "inv_hte_gate_note": "验证方式：{method}·holdout 样本量：{n}。只有 validation_ref 把配置的每道门槛全过了才配叫'验证过'：AUUC（排序质量）、校准误差（幅度诚实）、区间覆盖率（不确定性诚实——模型给的区间普遍过度自信，必须在 holdout 上自证覆盖率）。",
        "inv_th_validation_ref": "验证记录",
        "inv_th_learner": "模型",
        "inv_th_auuc": "Qini / AUUC",
        "inv_th_calibration_mae": "校准误差 (MAE)",
        "inv_th_coverage": "区间覆盖率",
        "inv_th_passes": "过门槛？",
        "inv_th_decile": "十分位",
        "inv_th_predicted_tau": "预测 tau",
        "inv_th_observed_lift": "实测提升",
        "inv_th_gap": "偏差",
        "inv_th_tau_bucket": "tau 区间",
        "inv_th_share": "人群占比",
        "inv_mmm_sub": "渠道级宏观模型摘要（PyMC-Marketing 风格），用来校准 SKU 级计划。",
        "inv_th_channel": "渠道",
        "inv_th_contribution": "渠道贡献",
        "inv_th_roas": "后验 ROAS",
        "inv_th_interval": "90% 区间",
        "inv_mmm_adstock": "广告残留 / 响应曲线",
        "inv_mmm_saturation": "饱和曲线",
        "inv_mmm_lift_calibration": "增量校准",
        "inv_default_owner": "待指派",
        "inv_default_flight": "下个规划周期",
        "inv_default_kpi": "增量销量",
        "inv_default_stop_rule": "实测边际投产比跌破达标线即暂停。",
        "inv_default_why": "边际投产比过了预算门槛，所以批钱。",
        "inv_limits_heading": "这份报告的限制——以及补齐哪些数据，下一轮才算得动",
        "inv_limits_intro": "批钱的 {funded} 个格子里，{assumption} 个还是假设级；另有 {blocked} 个格子完全没法批；holdout 样本量 = {holdout}。这些数字只能指导排期，不能当证据。",
        "inv_limits_blocked_line": "没批钱的格子：",
        "inv_csv_heading": "CSV 补数模板——填好回传，下一轮自动重算",
        "inv_csv_hint": "已预填今天的格子；空着的字段就是缺的数据。tau_source 只能填 randomized_hte / modeled_hte / mmm_calibrated_prior / expert_assumption；填 randomized_hte 的行必须同时给 validation_ref。",
        "inv_csv_download": "下载 CSV",
        "inv_csv_copy": "复制",
        "stance_invest": "投多",
        "stance_test": "试投",
        "stance_hold": "暂不投",
        "stance_fix": "先修再投",
        "inv_frontier_by_group": "同一条曲线，分产业拆开看",
        "inv_frontier_group_sub": "每张小图：该业务线投入增加时累计毛利怎么涨——比形状就知道哪条线还有空间。",
        "dash_tiers_drill_hint": "点开价格带看里面的 SKU；再点 SKU 看它的打法和这轮批的钱。",
        "tier_spend_word": "本轮批钱",
        "tier_sku_count_word": "个 SKU",
        "cat_bridge_ch1": "先给结论。下面每一章回答一个追问：为什么、对谁打、怎么落地、凭什么信。",
        "cat_bridge_ch2": "上一章说了投谁投多少——这一章摊开账本：类目哪里有问题，钱为什么恰好停在那条线。",
        "cat_bridge_ch3": "账算清了，落到打法：钱进哪个段位、哪个 SKU、哪个模块。",
        "cat_bridge_ch4": "打法定了，这一章是执行：每张卡一个负责人、一条止损线，最后是下一轮要补的数据。",
        "cat_bridge_ch5": "最后是证据与风险：哪些数字站得住、哪些还是假设——默认收起，点开细看。",
        "post_treatment_warn": "⚠ 处理后变量风险",
        "post_treatment_capped_note": "带 ⚠ 的维度：其代理变量条件在曝光后的行为（或对撞节点）上——热力图评分已封顶到'测试'档：这里看到的提升可能是自选择，不是因果。",
        "dash_4p_product": "产品",
        "dash_4p_price": "价格",
        "dash_4p_place": "渠道",
        "dash_4p_promotion": "推广",
        "verdict_grow": "加大投入",
        "verdict_hold": "维持",
        "verdict_harvest": "收割",
        "verdict_exit": "退出/规划接替",
        "dash_hero_portfolio": "组合判定",
        "dash_top_action": "头号动作",
        "dash_top_blocker": "头号卡点",
        "dash_detail_title": "详情",
        "dash_detail_empty_title": "点一张卡片",
        "dash_detail_empty_hint": "点击任意 KPI、渠道、热力格、任务卡、诊断或证据行，这里显示它的完整理由和数据。",
        "dash_kpi_skus": "SKU 总数",
        "dash_kpi_grow": "加大投入",
        "dash_kpi_exit": "退出",
        "dash_kpi_missing": "缺数据",
        "dash_tiers_title": "钱在哪个段位（价格带地图）",
        "dash_tiers_sub": "每个价格带：趋势、谁在买、什么能打动他们、对手是谁。",
        "dash_diag_title": "哪里有问题、代价是什么（类目诊断）",
        "dash_diag_sub": "发现 → 证据 → 不管的代价 → 对应打法；严重度按证据等级封顶。",
        "dash_skus_title": "每个 SKU：判定与打法",
        "dash_skus_sub": "按判定分组——加大投入在前、退出在后；点卡片看完整理由。",
        "dash_handoff_title": "下一步：送入单品深度分析",
        "dash_handoff_sub": "判定加大投入的 SKU——确认后逐个送入单品分析流程。",
        "dash_evidence_title": "证据台账",
        "dash_evidence_sub": "查证过的、假设的、算出来的、还没拿到的——绝不编数。",
        "dash_economics_title": "这笔账（单位经济）",
        "dash_economics_sub": "单位毛利、获客成本红线、敏感性杠杆。",
        "dash_channels_title": "渠道筛查",
        "dash_channels_sub": "放钱之前，哪些渠道站得住脚。",
        "dash_dimensions_title": "人群分组（D 维度）",
        "dash_dimensions_sub": "在上面这些渠道里，哪类买家更可能被打动。",
        "dash_heatmap_title": "打法矩阵（渠道 × 人群）",
        "dash_suppression_title": "绝对不碰的人",
        "dash_suppression_sub": "对这些人投放是烧钱——排除本身就是打法的一部分。",
        "dash_treatments_title": "任务卡与放行条件",
        "dash_treatments_sub": "每个动作都带机制、护栏、试验和放行条件。",
        "dash_budgets_title": "预算分批放行",
        "dash_budgets_sub": "钱分阶段解锁，每一笔都写明放行条件。",
        "dash_gates_sub": "预算解锁前必须落实的事。",
        "dash_challenges_title": "自我挑刺",
        "dash_challenges_sub": "独立评审对本分析的质疑——未解决的会冻结相关投放。",
        "dash_map_title": "因果链路",
        "dash_map_sub": "整个看板遵循的推理顺序，始终可见。",
        "dash_measurement_title": "测量计划",
        # 人群卡（ref 18：定义 → 定盘子 → 触达）
        "aud_heading": "对谁、多大盘子、怎么触达（人群卡）",
        "aud_intro": "渠道筛选说的是「在哪买流量」，这些卡说的是「把流量对准谁」：每个人群先定义、再定盘子——盘子是一条链算出来的，不是拍一个数；每个数打 A–D 等级；再给一条触达路径，并如实标注它够不够准。",
        "aud_role_word": "因果角色",
        "aud_role_persuadable": "可说服——值得花钱的就是这拨人",
        "aud_role_sure_thing": "反正都会买——别投，排除",
        "aud_role_lost_cause": "怎么都不买——投了也白投",
        "aud_role_sleeping_dog": "越投越糟——碰了反而掉转化",
        "aud_role_unknown": "还不知道——得做留出组才能判",
        "aud_who_word": "这拨人是谁",
        "aud_size_word": "盘子多大",
        "aud_reachable_word": "可达盘子",
        "aud_persuadable_word": "其中可说服",
        "aud_persuadable_unknown": "可说服占比未知——放量前先做一次留出组",
        "aud_grade_word": "等级",
        "aud_worst_grade_note": "整张卡的等级取最弱那一环。",
        "aud_factor_country_reachable": "本国可达人数",
        "aud_factor_category_or_intent_share": "× 在品类里/正在比价",
        "aud_factor_activation_match_rate": "× 代理真正能触达的比例",
        "aud_factor_eligibility_rate": "× 有资格（能发货、合规）",
        "aud_factor_persuadable_share": "× 可说服占比（因果）",
        "aud_reach_word": "怎么触达",
        "aud_th_platform": "平台",
        "aud_th_proxy": "定向代理",
        "aud_th_match": "匹配度",
        ...
        "aud_mq_medium": "中",
        "aud_mq_low": "低",
        "aud_mq_unavailable": "投不了",
        "aud_suppression_word": "排除",
        "aud_measurement_word": "怎么测",
        "aud_weakest_word": "最弱假设",
        "aud_grade_legend": "等级：A 查证过 · B 品类/搜索数据 · C 三方推算 · D 纯假设（D 级绝不写精确百分数）。",
        "aud_caption": "看图：盘子从左往右一层层收窄；右边的数就是可达盘子，它的徽章是整条链里最弱的等级。可说服占比只能来自留出组，绝不来自平台人群包。",
        "dash_audience_title": "人群卡",
        "dash_audience_sub": "对谁、盘子多大、怎么触达——盘子按链条算、逐环打等级。",
    },
}
