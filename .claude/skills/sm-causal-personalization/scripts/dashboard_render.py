    ...
    for row in rows:
        ...
        for col, grade in zip(columns, row.get("grades", [])):
            ...
            body.append(
                ...
                f"data-detail='{_detail_json(channel.get('name', ''), cell_text, detail)}'>"
                f"{_esc(grade)}</button>"
            )
    return _section(
        "heatmap",
        "Heatmap",
        "Click a cell to inspect the causal hypothesis, proxy, and validation action.",
        [f"<div class='heatmap' style='--cols:{len(columns)}'>{head}{''.join(body)}</div>"],
    )
