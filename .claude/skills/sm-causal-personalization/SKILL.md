...
| `investment_schema.py` / `hte_core.py` / `investment_engine.py` / `investment_charts.py` | Budget-frontier allocation engine for `cfg["investment_plan"]` — domain-agnostic (no brand/category/country name anywhere in these files). Schema validates allowed modules, forbidden discount-style levers, hard-excludes any SKU verdicted harvest/exit, and requires `validation_ref` for randomized HTE cells; `hte_core.py` gates "validated" confidence on supplied Qini/AUUC + decile calibration evidence; engine funds descending-marginal-ROI spend steps from a saturating response curve until the required-ROI floor or the budget stops it; charts build KPI cards, HTE core charts, a two-panel frontier, an SKU × module matrix, MMM specs, and activation-card inputs. `confidence` (validated / mmm-calibrated / assumption-grade / blocked) is computed, never a raw config field | Ch1 (verdict) / Ch2 (HTE + frontier) / Ch3 (matrix) / Ch4 (activation cards) / Ch5 (confidence + MMM) |
