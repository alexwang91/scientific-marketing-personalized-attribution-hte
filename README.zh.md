...
| `generate_report.py` | 决策报告生成器（v3）。**强制 provenance 合约**；**按读者五问组织**（花不花钱/账怎么算/对谁打/谁执行/凭什么信），内置中英人话文案包（`meta.lang` 切换）、统一任务卡（负责人/截止/止损线）；5 张因果逻辑交互图；`--format dashboard` 出交互驾驶舱；三档深度 `--depth quick/standard/deep` | **HTML 输出入口** |
